...
def writeRDBMSData(df,propfile,db,tbl,mode):#configuration driven approach
    config = ConfigParser()#instantiating the class ConfigParser as an object config
    config.read(propfile)#because i want to use a function called read to read the property file
    driver=config.get("DBCRED", 'driver')#and to use a function called get function to get the respective value of a given key
