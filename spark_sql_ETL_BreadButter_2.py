...
#Usecase: Consumers (Datascience Team):  source system (evolving data) -> data engineering team (evolving hive table loading) -> datascience team (consuming the evolving data)
...
# DSL (lakhouse)
flattened_denormalized_trans_cust_persist=cust_dim.alias("c").join(txns_fact.alias("t"),on=[col("c.id")==col("t.custid")],how="full")
...
print("d. Windowing Functionalities - Application of Windowing Functions 1. Surrogate key, 2. Top 'N' Analysis 3.Deduplication of data based on any attributes")
...
print("Month over month accross the customers whether the sales improved?")
cust_mth_sumamt=transamt_order_txns3.groupBy("custid",month(to_date("dt",'MM-dd-yyyy')).alias("mth")).agg(sum("amt").alias("sumamt")).orderBy("custid","mth")
cust_mth_sumamt.withColumn("priormthamt",lag("sumamt",1,-1).over(Window.partitionBy("custid").orderBy("custid","mth"))).show()

#DSL Equivalent with window functions
#DSL Equivalent with out using window functions #dropduplicates
#SQL Equivalent

#Just for Aravind's question about CTE (common table expression), we are doing this iteration
#inlineview (a from clause subquery)
spark.sql("select max(samt) from (select custid,sum(amt) as samt from view1 group by custid) inlineview").show()
#CTE - "with name as (query)" is a common table expression - help reduce the rewriting of the same query again and again
spark.sql("""with cte as (select custid,avg(amt) as aamt,max(amt) mamt from view1 group by custid) 
             select custid,aamt commonamt,case when aamt>0 then 'avgamt' else '' end as amttype from cte
             union
             select custid,mamt commonamt,case when mamt>0 then 'maxamt' else '' end as amttype from cte""").show()

#inlineview (a from clause subquery) - costly to write as given below
spark.sql("""select * from (select custid,sum(amt) as samt from view1 group by custid) inlineview1
             union all
             select * from (select custid,sum(amt) as samt from view1 group by custid) inlineview2 where custid=4000001""").show()
...
#please try out, however I am going to give as a complete sql query for the Bread and butter 2
...
#munged data
munged_df.write.mode("overwrite").saveAsTable("default.hive_trans_munged")
...
#Pls get me the technical terminologies (munging, wrangling), functions (window - rank, row_num...) and the business terminologies () used in this 2 programs bread and butter 1 & 2...